//huang2 and minError(I) give very similar thresholds that includes more matrin3 inside
//default and huang similar more stringent but miss stuff inside
...
//		run("Create Selection");
		...
		selectWindow("dapi_zproj");
		run("Duplicate...", "title=gblur_small");
		...
		rename("gblur_small_mask");
		run("Invert");
		run("Fill Holes");
		run("Adjustable Watershed", "tolerance=6");
//		run("Adjustable Watershed", "tolerance=2");
		run("Analyze Particles...", "size=20-Infinity show=Masks");
		rename("dapi_gblursmall");
//		run("Invert");
		run("Create Selection");
//		run("Make Inverse");
		roiManager("Add");
		selectWindow("gblur_small_mask");
		close();
		selectWindow("gblur_small");
		close();
		selectWindow("dapi_gblursmall");
		run("Select None");		
...
//		run("3D Nuclei Segmentation (beta)", "auto_threshold=Default manual=1");
		...
		run("Gaussian Blur 3D...", "x=12 y=12 z=3");
...
//fish default autothresh gave background, non-background ones are intermodes (low), isodata (high), max entropy (high), moments (too high), otsu, renyi entropy (higher), yen (middle), going with yen
//on weak FISH (17_3 from 1-22-21, intermodes (5), maxentropy (1 (too low)), moments (too high), renyientropy (too low), yen (too low)
		selectWindow("fish");
		run("Subtract Background...", "rolling=3 sliding stack");
		run("Duplicate...", "title=fish_gblur4 duplicate");
		run("Gaussian Blur 3D...", "x=3 y=3 z=2");
		selectWindow("fish");
		run("Duplicate...", "title=fish_gblur8 duplicate");
		run("Gaussian Blur 3D...", "x=6 y=6 z=3");
		imageCalculator("Subtract create stack", "fish_gblur4","fish_gblur8");
		run("Auto Threshold", "method=Moments ignore_black ignore_white white stack use_stack_histogram");
//		run("3D Nuclei Segmentation (beta)", "auto_threshold=Default manual=1");
		run("3D Simple Segmentation", "low_threshold=1 min_size=0 max_size=-1");
		selectWindow("Bin");
		close();
...
//		}
		...
		Ext.Manager3D_SaveQuantif("C:/FIJI/empfiles/Results.tsv");
		Ext.Manager3D_CloseResult("Q");
		open("C:/FIJI/empfiles/Q_Results.tsv");
		selectWindow("Q_Results.tsv");
		for (i=0;i<nb;i++) {
			sc35_cent_in[i] = Table.get("AtCenter", i);
		}
		selectWindow("Q_Results.tsv");
		run("Close");
		File.delete("C:/FIJI/empfiles/Q_Results.tsv");
		...
		selectWindow("Q_Results.tsv");
		for (i=0;i<nb;i++) {
			sc35_cent_out[i] = Table.get("AtCenter", i);
		}
		selectWindow("Q_Results.tsv");
		run("Close");
		File.delete("C:/FIJI/empfiles/Q_Results.tsv");
		...
		selectWindow("Q_Results.tsv");
		for (i=0;i<nb;i++) {
			matrin_cent_in[i] = Table.get("AtCenter", i);
		}
		selectWindow("Q_Results.tsv");
		run("Close");
		File.delete("C:/FIJI/empfiles/Q_Results.tsv");
		...
		selectWindow("Q_Results.tsv");
		for (i=0;i<nb;i++) {
			matrin_cent_out[i] = Table.get("AtCenter", i);
		}
		selectWindow("Q_Results.tsv");
		run("Close");
		File.delete("C:/FIJI/empfiles/Q_Results.tsv");
